...
void TaskMovePlate(void *pvParameters);
...
AccelStepper rotate = AccelStepper(motorInterfaceType, stepPin, dirPin);
...
//Set up motor-------------------------------------------------------------------------------------------------------------------
  ...
  // Set the maximum speed and acceleration for stepper and rotate:
  stepper.setMaxSpeed(99999999);
  stepper.setAcceleration(999999);
  rotate.setMaxSpeed(16000);
...
//Linear actuator set up---------------------------------------------------------------------------------------------------------------
  ...
  Serial.println("Suspend rotate, motor and gyro tasks, ready for input");
...
void TaskMotor(void *pvParameters) {
  ...
  for (;;) {
    ...
    Serial.println("15 seconds until recording resumes");
    ...
    if (Serial.available() > 0) {
      ...
      //send "16" to suspend Gyro and Motor tasks and resume/start Rotate
...
void TaskRotate(void *pvParameters) {
  ...
  for (;;){
      ...
      if (incomingByte < 20 && incomingByte > 0) {
        //send "12" to turn on power to stepper motor
        if (incomingByte == 12) { 
        digitalWrite(stepperMotorSignal, LOW);
        Serial.println("Stepper Motor power on"); 
